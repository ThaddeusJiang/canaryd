defmodule Canaryd.BuildCleanup do
  ...
  defp cleanup_sccache(result, home, cutoff, options) do
    cache =
      Canaryd.SccacheCleanup.run(home, cutoff,
        activity_scanner:
          Keyword.get(options, :sccache_activity_scanner, &Canaryd.SccacheCleanup.scan_activity/0)
      )

    %{
      result
      | sccache: cache,
        reclaimed_bytes: result.reclaimed_bytes + cache.reclaimed_bytes,
        failures: result.failures ++ cache.failures,
        skipped: Map.put(result.skipped, :sccache, cache.skipped)
    }
  end
