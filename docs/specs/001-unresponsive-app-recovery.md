...
Detect third-party GUI apps that macOS marks as not responding.
Restart a confirmed unresponsive app without user action.
...
- Out of scope:
  - Background daemons and helper processes.
  - Apple system apps.
...
3. Keep only apps with a regular activation policy.
4. Exclude Apple bundle identifiers and app bundles under system paths.
5. Confirm an unresponsive app in two consecutive check rounds.
6. Send `SIGTERM` to the confirmed process.
7. Send `SIGKILL` only when the same process does not stop within the grace period.
8. Wait for the old PID to stop.
9. Open the same app bundle in the background.
10. Log detection, restart, restart failure, and blocked events.
11. Notify the user only when restart fails or the app stays unresponsive during cooldown.
