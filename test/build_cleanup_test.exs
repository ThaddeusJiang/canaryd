defmodule Canaryd.BuildCleanupTest do
  ...
  test "sccache uses the saved retention and reports objects separately", %{root: root} do
    now = ~U[2030-01-01 00:00:00Z]
    path = Path.join([root, "Library/Caches/Mozilla.sccache/a/a", String.duplicate("a", 64)])
    File.mkdir_p!(Path.dirname(path))
    File.write!(path, "cached")
    {:ok, stat} = File.stat(path, time: :posix)
    old = DateTime.to_unix(now) - 36 * 3600
    File.write_stat!(path, %{stat | atime: old, mtime: old}, time: :posix)
    assert {:ok, _} = Canaryd.BuildCleanupConfig.set("48h", root)
    opts = [now: now, rust_roots: [], sccache_activity_scanner: fn -> {:ok, MapSet.new()} end]
    assert run(root, opts).sccache.removed_objects == 0
    assert File.exists?(path)
    assert {:ok, _} = Canaryd.BuildCleanupConfig.set("24h", root)
    result = run(root, opts)
